...
for (let mult = 1; mult <= 10; mult++) {
  //for(initilize iterator; iterator conditon( if its true then only the loop will run); iteraator increment)
...
const startNum = 16;
console.log(
  "Terminate when the number is divisible by both 3 and 5. Starting from ",
  startNum
);
...
numMul = startNum;
while (!(numMul % 3 == 0 && numMul % 5 == 0)) {
  //until the condition is true loop will run
  console.log(numMul);
  numMul++;
}
