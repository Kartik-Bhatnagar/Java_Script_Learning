...
const userChoice = prompt(
  "Enter what do you want to know about Himachal Pradesh \n area,population,distt,gdp"
);
...
const validYear = (givenBYear) =>
  givenBYear > 0 && givenBYear <= 2023 ? givenBYear : 2000; // if the given birth year is not valid then by default it
//will be consider as 2000
bYear = validYear(bYear);
console.log(`${person1.name} is ${person1.age(bYear)} years old and he has ${
  person1.friends.length
} friends
. He is ${
  person1.dLicence(person1.age(bYear)) ? "eligible" : "not eligible"
} for driving `);
...
console.log(
  `using 'this' ${
    person1.name
  } age is property function age : ${person1.calcAge()}\n property name age:  ${
    person1.personAge
  }`
);
...
person1["getSummary"] = function () {
  return `${this.name} was born in the year ${
    this.birthYear
  } and toady his age is ${this.personAge}.\n
     He ${
       this.personAge ? "eligible" : "not eligible"
     } for driving. He is having ${this.friends.length} friends and they are
     ${this.friends}`;
};
